...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| 2.3 | 2026-03-03 | 新增2.20按钮编辑模式模块（拖拽排序、右键菜单、Ctrl+C/V、JSON配置驱动）；新增2.21 Ghost Downloader下载器集成模块；补充2.4设备操作（ADB Root & Remount、Reboot to Loader）；更新3.7快捷按键配置系统已实现项 |
...
| 多模式重启 | 支持普通重启、Recovery、Bootloader、Loader | ✅ 已实现 |
| 输入模拟 | 模拟点击、滑动、按键输入 | ✅ 已实现 |
| ADB Root & Remount | 执行 adb root 并 adb remount，用于获取Root权限 | ✅ 已实现 |
...
| 功能 | 描述 | 状态 |
|------|------|------|
| 编辑模式开关 | 顶栏 ✏️ Edit 按钮一键切换编辑模式（完成后点 ✅ Done 退出） | ✅ 已实现 |
| 按钮拖拽排序 | 编辑模式下拖拽按钮在组内重新排列（DraggableButton） | ✅ 已实现 |
| 分组拖拽重排 | 拖动分组标题栏调整分组顺序（DraggableGroupBox） | ✅ 已实现 |
| 按钮跨组拖拽 | 将按钮拖拽到其他分组 | ✅ 已实现 |
| 右键上下文菜单 | 右键按钮弹出菜单：上移/下移/移动到组/复制到组/删除 | ✅ 已实现 |
| Ctrl+C/V 复制粘贴 | 编辑模式下支持复制按钮并粘贴到目标组 | ✅ 已实现 |
| JSON配置文件驱动 | 按钮布局由 buttons_cmd.json 文件定义，修改后持久化保存 | ✅ 已实现 |
| 按钮参数化 | 按钮支持 file/folder/text 类型参数，点击时弹出输入对话框 | ✅ 已实现 |
| 编辑态样式高亮 | 编辑模式下按钮显示特殊边框样式以区分编辑状态 | ✅ 已实现 |

### 2.21 Ghost Downloader 下载器集成模块

| 功能 | 描述 | 状态 |
|------|------|------|
| 嵌入式下载器Tab | Ghost Downloader 以独立Tab页（📥 下载器）嵌入主界面 | ✅ 已实现 |
| 完整导航界面 | 加载 EmbeddedMainWindow，包含 Ghost Downloader 完整导航栏 | ✅ 已实现 |
| WebSocket通信 | 通过 ws://127.0.0.1:14370 与后台下载服务通信 | ✅ 已实现 |
| 自动服务启动 | 检测到服务未运行时自动在后台无窗口启动下载服务 | ✅ 已实现 |
| 服务就绪等待 | 最多等待15秒确认 WebSocket 服务就绪 | ✅ 已实现 |
| 异步下载任务 | 后台线程提交下载任务，不阻塞主界面 | ✅ 已实现 |
| 独立窗口回退 | 嵌入组件加载失败时回退为独立窗口打开 | ✅ 已实现 |
| 插件配置集成 | 通过 plugins_config.json 配置 Ghost Downloader 路径和参数 | ✅ 已实现 |
...
| 功能 | 描述 | 优先级 |
|------|------|--------|
| 配置文件定义 | 通过JSON/YAML文件定义所有快捷按键 | P0 **✅ 已实现**（buttons_cmd.json） |
| 按键显示/隐藏 | 控制按键在界面上的可见性 | P0 |
| 按键启用/禁用 | 控制按键是否可点击 | P0 |
| 按键顺序调整 | 自定义按键在界面上的排列顺序 | P1 **✅ 已实现**（右键菜单上移/下移 + 拖拽） |
| 分组管理 | 按键按功能分组（如：设备操作、文件传输等） | P0 **✅ 已实现** |

#### 3.7.2 按键自定义
| 功能 | 描述 | 优先级 |
|------|------|--------|
| 修改命名 | 自定义按键显示名称（支持中英文） | P0 **✅ 已实现**（通过JSON配置文件修改 label 字段） |
| 修改图标 | 自定义按键Emoji图标 | P1 **✅ 已实现**（通过JSON配置文件修改 label 中的Emoji） |
| 修改执行功能 | 绑定到不同的ADB命令或Python函数 | P0 **✅ 已实现**（通过JSON配置文件修改 action 字段） |
| 修改快捷键 | 设置键盘快捷键触发 | P2 |
...
| 功能 | 描述 | 优先级 |
|------|------|--------|
| 新增按键 | 添加用户自定义按键 | P0 **✅ 已实现**（编辑模式 Ctrl+V 粘贴新增） |
| 删除按键 | 移除不需要的按键 | P0 **✅ 已实现**（编辑模式右键菜单删除） |
| 重置默认 | 恢复出厂默认按键配置 | P0 |
| 导入配置 | 从外部文件导入按键配置 | P1 |
| 导出配置 | 导出当前按键配置为文件 | P1 |
