...
class TaskCard(CardWidget, Ui_TaskCard):
    ...
    def __init__(self, url: str, fileName: str, filePath: str, preBlockNum: int, headers: dict, status: str,
        ...
        if not self.status == "finished":  # 不是已完成的任务才要进行的操作
            ...
            # TODO 超分辨率触发条件
            _ = QFileIconProvider().icon(QFileInfo(f"{self.filePath}/{self.fileName}")).pixmap(48, 48).scaled(128, 128, aspectRatioMode=Qt.AspectRatioMode.KeepAspectRatio,
                                   transformMode=Qt.TransformationMode.SmoothTransformation)  # 自动获取图标
    ...
    def __updateProgress(self, content: list):
        ...
        if self.ableToParallelDownload:
            ...
            for e, i in enumerate(content):
                progress_value = ((i["progress"] - i["start"]) / (i["end"] - i["start"])) * 100
    ...
    def __onTaskFinished(self):
        ...
        if self.status != "finished":  # 不是自动创建的已完成任务
            ...
            # 再获取一次图标
            fileinfo = QFileIconProvider().icon(QFileInfo(f"{self.filePath}/{self.fileName}")).pixmap(48, 48).scaled(128, 128, aspectRatioMode=Qt.AspectRatioMode.KeepAspectRatio,
                                   transformMode=Qt.TransformationMode.SmoothTransformation)  # 自动获取图标
