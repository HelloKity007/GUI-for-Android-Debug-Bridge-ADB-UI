...
# 后台服务模式：强制启用 WebSocket 并隐藏窗口
HEADLESS_MODE = "--headless" in sys.argv or "--service" in sys.argv
if HEADLESS_MODE:
    # 强制启用浏览器扩展（WebSocket服务）
    cfg.set(cfg.enableBrowserExtension, True)
    # 禁用剪贴板监听
    cfg.set(cfg.enableClipboardListener, False)
    # 禁用启动时检查更新
    cfg.set(cfg.checkUpdateAtStartUp, False)
...
if HEADLESS_MODE:
    logger.info("Running in HEADLESS mode - WebSocket enabled, window hidden")
...
# 后台服务模式：始终静默启动
SILENCE_MODE = HEADLESS_MODE or "--silence" in sys.argv
w = MainWindow(silence=SILENCE_MODE)
...
# 后台服务模式日志
if HEADLESS_MODE:
    logger.info("Ghost Downloader Headless Service Started - WebSocket on ws://127.0.0.1:14370")
